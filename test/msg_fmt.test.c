  ...
  FOREVER {
    ...
    if (online) {
      ...
      if (connected == FALSE) {
        ...
        msg_fmt_install(Validation, validation_perform_cb);
        msg_fmt_install(Monitoring, monitoring_perform_cb);
        msg_fmt_install(Deployment, deployment_perform_cb);
        msg_fmt_install(Update, update_perform_cb);
      }

      vTaskDelay( 250 / portTICK_RATE_MS);

      msg_raw_send(msg_raw_topic(Validation, TO_HUB), "{\"hub\":\"hub-abcd\",\"status\":\"success\"}");
      vTaskDelay(100 / portTICK_RATE_MS);
      msg_raw_send(msg_raw_topic(Monitoring, TO_HUB), "{\"hub\":\"hub-abcd\",\"status\":\"success\"}");
      vTaskDelay(100 / portTICK_RATE_MS);
      msg_raw_send(msg_raw_topic(Deployment, TO_HUB), "{\"hub\":\"hub-abcd\",\"status\":\"success\",\"resource\":1,\"action\":\"init\"}");
      vTaskDelay(100 / portTICK_RATE_MS);
      msg_raw_send(msg_raw_topic(Execution , TO_HUB), "{\"hub\":\"hub-abcd\",\"status\":\"success\",\"resource\":2,\"action\":\"start\"}");
      vTaskDelay(100 / portTICK_RATE_MS);
      msg_raw_send(msg_raw_topic(Update    , TO_HUB), "{\"hub\":\"hub-abcd\",\"url\":\"www.google.com\",\"action\":\"update\"}");
      vTaskDelay(100 / portTICK_RATE_MS);
    }

    vTaskDelay( 65000 / portTICK_RATE_MS);
  }
