...
static IoT_Error_t _aws_iot_mqtt_internal_subscribe(AWS_IoT_Client *pClient, const char *pTopicName,
	...
	//ESP_LOGE("[DEEP INTERNAL SUBSCRIBE]","SUBSCRIBING");
	...
	uint8_t existingSubCount = _aws_iot_mqtt_get_free_message_handler_index(pClient);
	//ESP_LOGW("[BASE INTERNAL RESUBSCRIBE]","Existing subscribes [%d]",existingSubCount);
	for(uint8_t itr = 0; itr < existingSubCount; itr++) {
		if(pClient->clientData.messageHandlers[itr].topicName == NULL) {
			ESP_LOGE("[Subscribing]","Topic null");
			continue;
		}else if(!strcmp(pClient->clientData.messageHandlers[itr].topicName,
		pTopicName)){
			ESP_LOGE("[Subscribing]","Already subscribed");
			FUNC_EXIT_RC(SUCCESS);
		}
	}
	...
	if(SUCCESS != rc) {
		//ESP_LOGE("[DEEP INTERNAL SUBSCRIBE]","error serializing [0x%X]",rc);
		FUNC_EXIT_RC(rc);
	}

	indexOfFreeMessageHandler = _aws_iot_mqtt_get_free_message_handler_index(pClient);
	//ESP_LOGE("[DEEP INTERNAL SUBSCRIBE]","freemsg handlers [0x%X]",indexOfFreeMessageHandler);
	if(AWS_IOT_MQTT_NUM_SUBSCRIBE_HANDLERS <= indexOfFreeMessageHandler) {
		//ESP_LOGE("[DEEP INTERNAL SUBSCRIBE]","error maxhandlers [0x%X]",indexOfFreeMessageHandler);
	...
	if(SUCCESS != rc) {
		//ESP_LOGE("[DEEP INTERNAL SUBSCRIBE]","error sending packet serializing [0x%X]",rc);
		FUNC_EXIT_RC(rc);
	}
	...
	if(SUCCESS != rc) {
		//ESP_LOGE("[DEEP INTERNAL SUBSCRIBE]","eRROR READING read subscribe [0x%X]",rc);
		FUNC_EXIT_RC(rc);
	}
...
IoT_Error_t aws_iot_mqtt_subscribe(AWS_IoT_Client *pClient, const char *pTopicName, uint16_t topicNameLen,
	...
	//ESP_LOGE("[INTERNAL SUBSCRIBE]","SUBSCRIBING");
	...
	if(!aws_iot_mqtt_is_client_connected(pClient)) {
		ESP_LOGE("[INTERNAL SUBSCRIBE]"," error disconnected ");
	...
	if(CLIENT_STATE_CONNECTED_IDLE != clientState && CLIENT_STATE_CONNECTED_WAIT_FOR_CB_RETURN != clientState) {
		ESP_LOGE("[INTERNAL SUBSCRIBE]"," error unidle [0x%X]",clientState);
	...
	if(SUCCESS != rc) {
		ESP_LOGE("[INTERNAL SUBSCRIBE]"," 1 Error in subscribing! [0x%X]",rc);
		FUNC_EXIT_RC(rc);
	}
	...
	if(SUCCESS == subRc && SUCCESS != rc) {
		ESP_LOGE("[INTERNAL SUBSCRIBE]"," 2 Error in subscribing! [0x%X]",rc);
		subRc = rc;
	}
...
static IoT_Error_t _aws_iot_mqtt_internal_resubscribe(AWS_IoT_Client *pClient) {
	//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","RESUBSCRIBING");
	uint16_t packetId;
	...
	for(itr = 0; itr < existingSubCount; itr++) {
		if(pClient->clientData.messageHandlers[itr].topicName == NULL) {
			//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","Resubsciption null");
			continue;
		}
		...
		if(SUCCESS != rc) {
			//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","Error in resubscribing! [0x%X]",rc);
			FUNC_EXIT_RC(rc);
		}
		...
		if(SUCCESS != rc) {
			//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","Error in SENDIN PACKET resubscribing! [0x%X]",rc);
			FUNC_EXIT_RC(rc);
		}
		...
		rc = aws_iot_mqtt_internal_wait_for_read(pClient, SUBACK, &timer);
		if(SUCCESS != rc) {
			//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","Error in READING PACKET resubscribing! [0x%X]",rc);
			FUNC_EXIT_RC(rc);
		}
		...
		rc = _aws_iot_mqtt_deserialize_suback(&packetId, 1, &count, grantedQoS, pClient->clientData.readBuf,
											  pClient->clientData.readBufSize);
		if(SUCCESS != rc) {
			//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","Error in DESIARILIZNG PACKET resubscribing! [0x%X]",rc);
			FUNC_EXIT_RC(rc);
		}
		//ESP_LOGE("[BASE INTERNAL RESUBSCRIBE]","Resubscribed!!!!!");
	}
...
IoT_Error_t aws_iot_mqtt_resubscribe(AWS_IoT_Client *pClient) {
	ESP_LOGE("[INTERNAL RESUBSCRIBE]","RESUBSCRIBING");
	...
	if(CLIENT_STATE_CONNECTED_IDLE != aws_iot_mqtt_get_client_state(pClient)) {
		ESP_LOGE("[INTERNAL RESUBSCRIBE]","Error CLIENT UNIDLE resubscribing!");
		FUNC_EXIT_RC(MQTT_CLIENT_NOT_IDLE_ERROR);
	}
	...
	if(SUCCESS != rc) {
		ESP_LOGE("[INTERNAL RESUBSCRIBE]","Error 1 CLIENT UNIDLE resubscribing!");
		FUNC_EXIT_RC(rc);
	}
	...
	if(SUCCESS == resubRc && SUCCESS != rc) {
		ESP_LOGE("[INTERNAL RESUBSCRIBE]","resubscribe not success");
		resubRc = rc;
	}
