...
IoT_Error_t aws_iot_mqtt_attempt_reconnect(AWS_IoT_Client *pClient) {
	ESP_LOGE("[attempt reconenct]","attempting reconnect");
	IoT_Error_t rc;
	...
	if(aws_iot_mqtt_is_client_connected(pClient)) {
		ESP_LOGE("[attempt reconenct]","network connected err");
		FUNC_EXIT_RC(NETWORK_ALREADY_CONNECTED_ERROR);
	}
	...
	if(CLIENT_STATE_CONNECTED_IDLE != aws_iot_mqtt_get_client_state(pClient)) {
		ESP_LOGE("[attempt reconenct]","state unidle");
	...
	ESP_LOGE("[attempt reconenct]","attempting resubscribe");
	rc = aws_iot_mqtt_resubscribe(pClient);
	if(SUCCESS != rc) {
		FUNC_EXIT_RC(rc);
	}
	ESP_LOGE("[attempt reconenct]","reconnected!!!!!");
