...
IoT_Error_t aws_iot_mqtt_internal_send_packet(AWS_IoT_Client *pClient, size_t length, Timer *pTimer) {
  Timer ack_auxtimer;
	size_t sentLen, sent;
	IoT_Error_t rc;
	init_timer(&ack_auxtimer);
	countdown_ms(&ack_auxtimer, 100);
	Timer* used_timer = pTimer;
...
static IoT_Error_t _aws_iot_mqtt_internal_readWrapper( AWS_IoT_Client *pClient, size_t offset, size_t size, Timer *pTimer, size_t * read_len ) {
    ...
    byteToRead = ( offset + size ) - pClient->clientData.readBufIndex;
		//ESP_LOGE("[MQTT-INTERNAL]","Reading internal packet readwrapper()");
    if ( byteToRead > 0 )
    {
			//ESP_LOGE("[MQTT-INTERNAL]","Reading internal packet readwrapper read()");
...
static IoT_Error_t _aws_iot_mqtt_internal_read_packet(AWS_IoT_Client *pClient, Timer *pTimer, uint8_t *pPacketType) {
  ...
  rc = _aws_iot_mqtt_internal_readWrapper( pClient, offset, 1, &packetTimer, &read_len );
...
static IoT_Error_t _aws_iot_mqtt_internal_handle_publish(AWS_IoT_Client *pClient, Timer *pTimer) {
 ...
 //ESP_LOGI("[LOW-LEVEL-MQTT]","New msg isdup: [%d],id: [%d]",msg.isDup,msg.id);
	if(SUCCESS != rc) {
		//ESP_LOGE("[LOW-LEVEL-MQTT]","Publish desiarlization ERROR");
		FUNC_EXIT_RC(rc);
	}
	...
	if(SUCCESS != rc) {
		ESP_LOGE("[LOW-LEVEL MQTT]","CANNOT ACK PACKAGE");
		FUNC_EXIT_RC(rc);
	}
	ESP_LOGI("[LOW-LEVEL MQTT]","Sending ACKED packet");
	FUNC_EXIT_RC(SUCCESS);
}
...
#endif
//ESP_LOGE("[MQTT-INTERNAL]","INTERNAL READ PACKET()");
...
#endif
//ESP_LOGE("[MQTT-INTERNAL]","Unlocked MUTEX()");
	...
	//ESP_LOGI("[LOW-LEVEL_MQTT]","Packet type [%d]",(*pPacketType));
	switch(*pPacketType) {
		case CONNACK:
		case PUBACK:
		case SUBACK:
		case UNSUBACK:
			...
			break;
		case PUBLISH: {
			//ESP_LOGE("[MQTT-INTERNAL]","trigger PUBLISH");
		...
		case PINGRESP: {
			...
			//ESP_LOGE("[MQTT-INTERNAL]","trigger PINGRESP");
